...
(function(){
  ...
  if(!c || !nav || prefersReducedMotion) return;
  const ctx = c.getContext('2d');
  let running = false;
  let rafId   = null;

  function resize(){
    c.width  = nav.offsetWidth;
    c.height = nav.offsetHeight;
  }
  resize();
  window.addEventListener('resize', resize, {passive:true});

  // 25 partículas (era 55) — visible igual, mucho más liviano
  const P = Array.from({length:25}, ()=>({
    x: Math.random() * (c.width  || 1200),
    y: Math.random() * (c.height || 70),
    r: Math.random() * 1.3 + .3,
    vx: (Math.random() - .5) * .35,
    vy: (Math.random() - .5) * .25,
    col: Math.random() > .5 ? '61,155,255' : '168,85,247',
    a: Math.random() * .45 + .1
  }));

  const LFPS=30, LINTERVAL=1000/LFPS;
  let lastLFrame=0;
  function draw(ts){
    if(ts-lastLFrame < LINTERVAL){ if(running) rafId=requestAnimationFrame(draw); return; }
    lastLFrame=ts;
    ...
    // Líneas con batching por alpha — evita ctx.stroke() por par
    const lineBuckets={};
    for(let i=0;i<P.length;i++) for(let j=i+1;j<P.length;j++){
      const d = Math.hypot(P[i].x-P[j].x, P[i].y-P[j].y);
      if(d < 70){
        const alpha=Math.round((1-d/70)*3)/3;
        if(!lineBuckets[alpha]) lineBuckets[alpha]=[];
        lineBuckets[alpha].push(P[i].x,P[i].y,P[j].x,P[j].y);
      }
    }
    ctx.lineWidth=.5;
    for(const alpha in lineBuckets){
      ctx.strokeStyle=`rgba(61,155,255,${alpha*.09})`;
      ctx.beginPath();
      const b=lineBuckets[alpha];
  ...
  // Observar cambios en la clase del nav
...
function openPfModal(id){
  ...
  document.getElementById('pfModalHint').textContent  = 'Agrega el video: ' + p.video;
  video.src = p.video;
...
function closePfModal(e){
  if(e && e.target !== document.getElementById('pfModalBg')) return;
  const bg    = document.getElementById('pfModalBg');
  const video = document.getElementById('pfVideo');
